...
\subtext{Pune, Maharashtra, India {\normalsize\normalfont (On-site, full-time) }} 
    \begin{zitemize}
        \item Co-created a real-time transaction authorization module for the \href{https://www.amazon.co.uk/dp/B0BH98211K}{\textbf{Amazon Barclaycard}}, achieving 70 transactions per second with a 60 ms P95 response time, leveraging \textbf{Java, Spring Boot, and Amazon Web Services(AWS)} for scalability and reliability.
        \item Engineered an optimized FCA-compliant Persistent Debt logic on the next-gen credit card platform by restricting evaluations to eligible accounts, resulting in 85\% memory and processing savings over the legacy system utilizing \textbf{AWS Lambda functions and ECS}.
        \item Implemented and maintained \textbf{CloudWatch alarms and metrics} for failure monitoring, with comprehensive technical documentation to support transparency and ongoing maintenance.
        \item Maintained high code quality through Test‑Driven Development(\textbf{TDD}) and Behavior-Driven Development(\textbf{BDD}) practices using \textbf{JUnit and Mockito}, ensuring robust, testable, and business-aligned implementations.
...
\subtext{Greater New York, USA {\normalsize\normalfont (Remote Internship) }} 
    \begin{zitemize}
        \item Engineered a secure E-KYC system using face recognition and optical character recognition(OCR) for identity card data extraction and user verification through recorded video and extracted identity card photo with a team of 4 developers.
...
\subtext{Gurgaon, India {\normalsize\normalfont (Remote Internship) }}
    \begin{zitemize}
        \item Designed a responsive web UI with a performance score of 90+ on Google Lighthouse using \textbf{HTML, CSS, Bootstrap, JS}, and packages like AOS, sweet-alert, and Glightbox. 
        \item Optimized homepage performance score on Desktop by 200\% using speed optimization strategies like \textbf{lazy loading, pre-fetching/pre-loading, caching} that helped improve page ranking on search engines.
        \item Streamlined lead tracking by integrating website enquiry forms with \textbf{Bitrix24 CRM using PHP}, improving response time.
    \end{zitemize}
...
\textbf{Programming Languages/Frameworks}: Java, Spring Boot, AWS SDK, tools and services, Junit, Mockito, Cucumber, ReactJS, SQL \\ 
\textbf{Key Modules}: Java Design Patterns, Data structures, SDLC, Operating System, Object Oriented Programming (OOP)
...
\begin{zitemize}
    ...
    \item Ranked \textbf{3/1600+} on Geeks on GeeksForGeeks platform 
