...
\subtext{Pune, Maharashtra, India {\normalsize\normalfont (On-site, full-time) }} 
    \begin{zitemize}
        \item Co-created a real-time transaction authorization module for the \href{https://www.amazon.co.uk/dp/B0BH98211K}{\textbf{Amazon Barclaycard}}, \textbf{achieving 70 transactions per second with a 60ms P95 response time}, leveraging Java, Spring Boot, and Amazon Web Services(AWS) for scalability and reliability.
        \item Implemented \textbf{credit card payment restrictions} based on account status, delivering 100\% of the feature suite with tests, monitoring, alarms, and failure handling.
        \item Contributed to the account swap feature, enabling seamless migration of 5,000+ users from the old to the new Amazon Barclaycard with minimal purchase impact.
...
\subtext{Greater New York, USA {\normalsize\normalfont (Remote Internship) }} 
    \begin{zitemize}
        ...
        \item Delivered a 4-step client-side form with custom drop zone, reactive OTP input, and video capture via MediaRecorder API.
